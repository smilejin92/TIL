...
콜백패턴, 후속 처리, 순서 보장, 예외 처리

1. Promise
2. Axios - 파싱을 내부적으로 해준다, IE 커버 (better than fetch-api)
3. fetch-api
4. async/await
...
자바스크립트의 대부분의 DOM 이벤트와 Timer 함수(setTimeout, setInterval), Ajax 요청은 비동기식 처리 모델로 동작한다. 비동기식 처리 모델은 요청을 병렬로 처리하여 다른 요청이 블로킹(blocking, 작업 중단)되지 않는 장점이 있다.
...
비동기 함수 내의 `readystatechange` 이벤트 핸들러에서 처리 결과를 반환(1)하면 순서가 보장되지 않는다. 즉, 2에서 `get` 함수가 반환한 값을 참조할 수 없다. 그 이유에 대해 살펴보자.
...
1. Promise 객체 생성, 내부에 비동기 처리를 구현
...
* Promise 객체는 상태를 갖는다 (fulfilled, rejected 등). 이 상태에 따라 후속 처리 메소드를 체이닝 방식으로 호출한다. Promise의 **후속 처리 메소드는 아래와 같다.**
