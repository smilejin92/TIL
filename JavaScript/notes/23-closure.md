...
1에서 배열 `arr`에는 5개의 함수가 요소로 추가된다. 그리고 2에서 배열 `arr`에 요소로 추가된 5개의 함수는 순차적으로 호출된다. 이때 `arr`에 요소로 추가된 5개의 함수가 각각 0, 1, 2, 3, 4를 반환할 것 같지만 그렇지 않다.

`for` 문의 초기화 문에서 `var` 키워드로 선언한 변수 `i`는 블록 레벨이 아닌 함수 레벨 스코프를 갖기 때문에 **전역 변수가 되며** `i`에는 0, 1, 2, 3, 4, 5가 순차적으로 할당된다.따라서 `arr`에 요소로 추가된 함수를 호출하면 전역 변수 `i`를 참조하여 `i` 값 5가 출력된다.
...
1에서 즉시 실행 함수는 전역 변수 `i`에 현재 할당되어 있는 값을 인수로 전달받아 매개 변수 `id`에 할당한 후 중첩 함수를 반환하고 종료된다. 이때 반환되는 중첩 함수는 배열 `arr`에 순차적으로 저장된다.

이때 즉시 실행 함수의 매개 변수 `id`는 반환된 함수의 상위 스코프에 존재하며, 반환된 함수는 `id`를 자유 변수로 참조하여 그 값이 유지된다.

위 예제는 자바스크립트의 함수 레벨 스코프 특성으로 인해 `for`문의 초기화 문에서 `var` 키워드로 선언한 변수가 전역 변수가 되기 때문에 발생하는 현상이다. ES6의 `let` 키워드를 사용하면 이와 같은 번거로움이 깔끔히 해결된다.
...
**`for`문의 초기화 문에서 `let` 키워드로 선언한 변수를 사용하면 `for`문이 반복될 때마다 `for`문 코드 블록의 새로운 렉시컬 환경이 생성된다.** 만약 `for`문 내에서 정의된 함수가 있다면 이 함수의 상위 스코프는 `for`문이 반복될 때마다 생성된 `for`문 코드 블록의 새로운 렉시컬 환경이다.
...
3. 2와 동일
4. 2와 동일
5. `for` 문의 반복이 모두 종료되면 `for`문이 실행되기 이전의 **렉시컬 환경을 Running execution context의 렉시컬 환경으로 되돌린다.**

이처럼 `var` 키워드로 사용하지 않은 ES6의 반복문(`for..in`문, `for..of`문, `while`문 등)은 반복할 때마다 새로운 렉시컬을 생성하여 반복할 당시의 상태를 마치 스냅샷을 찍는 것처럼 저장한다. **단, 이는 반복문 내부에서 함수 정의가 존재할 때 의미가 있다.**
