...
그 이유는 **개발자가 직접 추가한 메소드와 미래에 표준 사양으로 추가될 메소드 이름이 중복될 수 있기 때문**이다. 예를 들어, `Array.prototype.find` 메소드가 ES6에 새롭게 도입되기 전에 `Array.prototype`에 사용자 정의 `find` 메소드를 직접 추가했다면 새롭게 도입된 ES6의 `find` 메소드와 이름이 중복되어 사용자 정의 메소드가 오버라이딩한다.
