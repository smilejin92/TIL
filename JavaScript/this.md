...
* 메소드는 자신이 속한 객체의 프로퍼티(상태)를 참조하고 변경할 수 있어야 한다. 따라서 **메소드는 자신이 속한 객체를 가리키는 식별자를 참조할 수 있어야 한다.**
...
* **this가 가리키는 값(this binding)은 함수 호출 방식에 의해 동적으로 결정된다.**
...
* 함수의 종류에 관계 없이 **일반 함수로 호출되면 함수 내부의 this에는 전역 객체가 바인딩된다.**
* 객체를 생성하지 않는 **일반 함수에서 this는 의미가 없다**(this는 객체의 프로퍼티나 메소드를 참조하기 위한 자기 참조 변수이기 때문)
* 외부 함수(ex. 메소드)가 중첩 함수 혹은 콜백 함수를 일반 함수로 호출하면 각자 내부에서 참조하는 this가 불일치한다. 이것은 문맥이 맞지 않는 심각한 문제이다.
...
* 객체와 객체의 프로퍼티가 가리키는 함수(메소드) 객체는 별개다. 따라서 객체의 메소드를 다른 객체의 메소드로 추가할 수 있고, 일반 변수에 할당하여 일반 함수로도 호출 할 수 있다.
...
* `Function.prototype.apply`, `Function.prototype.call` 메소드는 인수로 this와 인수 리스트를 전달 받아 함수를 호출한다.
