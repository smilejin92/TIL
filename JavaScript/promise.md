...
자바스크립트는 비동기 처리를 위한 하나의 패턴으로 콜백 함수를 사용한다. 하지만 전통적인 콜백 패턴은 가독성이 떨어지며 비동기 처리 중 발생한 에러의 예외 처리가 곤란하다. 또한 콜백 패턴으로 여러 개의 비동기 처리를 한 번에 처리하는 것도 한계가 있다. ES6에서는 비동기 처리를 위한 또 다른 패턴으로 프로미스(Promise)를 도입하였다. 프로미스는 전통적인 콜백 패턴이 가진 단점을 보완하며 비동기 처리 시점을 명확하게 표현할 수 있는 장점이 있다.
...
> 이벤트 핸들러도 함수이므로 이벤트 핸들러의 평가 -> 이벤트 핸들러의 실행 컨택스트 생성 -> 콜 스택에 푸시 -> 이벤트 핸들러의 실행 단계를 거친다.
