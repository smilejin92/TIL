...
[동영상 강의](https://www.inflearn.com/course/%EC%83%9D%ED%99%9C%EC%BD%94%EB%94%A9-%EB%A6%AC%EB%88%85%EC%8A%A4-%EA%B0%95%EC%A2%8C)
...
`ls -l 1> result.txt` ls -l의 결과를 result.txt에 저장하라 (redirection `1>`, 콘솔에 안찍히고 파일로 출력 후 저장)

`cat result.txt` result.txt를 출력하라 (using standard input)

`cat > result.txt` result.txt를 출력하라 (using result.txt as parameter)



#### standard error

`rm rename2.txt 2> error.log` rm 명령에서 생긴 오류를 error.log에 저장



#### append

`ls -al >> result.txt` ls - al의 결과를 덮어쓰지말고 result.txt 본문 뒤에 더해서 저장
...
![](C:\Users\Jinhyun Kim\Documents\dev\TIL\shell command on Linux\images\filesystem-structure.png)
...
``` shell
locate *.log (이 PC에 .log로 끝나는 모든 파일의 디렉토리를 출력, mlocate DB를 query하여)
sudo updatedb (mlocate DB 업데이트)
find / -name *.log (이 PC에 .log로 끝나는 모든 파일의 디렉토리를 root에서 부터 탐색, 출력)

whereis ls (locates the binary, source and manual files for specified command names)
whereis rm
```
...
``` shell
ps (실행 중인 프로세스를 출력, top, htop과 비슷함)
ps aux | grep apache (백그라운드에서 실행 중인 모든 프로세스 중, apache라는 이름을 가진 프로세스를 출력)
sudo kill 22142 (22142 PID를 강제 종료)
```
...
![](C:\Users\Jinhyun Kim\Documents\dev\TIL\shell command on Linux\images\permission.PNG)
...
![](C:\Users\Jinhyun Kim\Documents\dev\TIL\shell command on Linux\images\chmod.PNG)
...
![](C:\Users\Jinhyun Kim\Documents\dev\TIL\shell command on Linux\images\group-permission.png)
...
![](C:\Users\Jinhyun Kim\Documents\dev\TIL\shell command on Linux\images\internet-1.png)
